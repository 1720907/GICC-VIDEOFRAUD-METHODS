...
from tensorflow.keras.applications import NASNetLarge as VGG16
...
def preprocess_video(video_path, counter):
  ...
  while True:
    ...
    resized_frame = cv2.resize(frame, (128, 128)) #,interpolatiod = cv2.INTER_AREA
...
## Calculate (lb, ub) f.
def calculate_lb_ub(sod_videos): 
  i=1
  thresholds = []
  sigma_coefficient = 3.5
  for sod in sod_videos:
    mean_sod = np.mean(sod) # Mean of second derivatives
    std_sod = np.std(sod, ddof=1) # standard dev. of second derivatives.
...
# 4.4 VFD & VFC
def vf_detection(correlation_diff, lambda_t):
      return np.array(correlation_diff) > lambda_t

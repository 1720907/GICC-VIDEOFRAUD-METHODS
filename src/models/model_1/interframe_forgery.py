...
def generate_frame_deletions(frames_to_delete, total_frames, percent_deletion):
  #pregunta abierta: ¿cuál es el mejor parámetro para el nro de eliminaciones (20%, 30%, etc)?: no nos cerramos a un nro, sino que probamos varios parámetros...
  ...
  max_deletions = round((frames_to_delete/10)+0.5) #redondeo para arriba
  number_fill_ranges=max_deletions+1
  min_separation= round((left_after_delete/number_fill_ranges)-0.5) #redondeo para abajo
...
def run():
  ...
  # Obtener la lista de archivos en input_path y ordenarla alfabéticamente

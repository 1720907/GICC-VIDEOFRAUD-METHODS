...
def parse_args():
    ...
    parser.add_argument('-lt','--lambda_t', type=float, default=0.08, help='Lambda threshold for determine a video is original/forged for VFD')
    parser.add_argument('-sc','--sigma_c', type=float, default=3.5, help='Sigma coefficient for calculating adaptive thresholds for VFC')
...
def run():
  ...
  for seed in seeds_to_test:
    ...
    if not os.path.exists(new_seed_path) or not isdir(new_seed_path):

...
def run():
  ...
  for fold, (train_indices,test_indices) in enumerate(skf.split(np.arange(total_videos), Y),1):
    ...
    # Training
    with device:
      checkpoint = ModelCheckpoint(checkpoint_path+f"/model_fold_{fold}.hdf5",monitor = 'accuracy', save_best_only = True, mode='max', verbose=0)
      # early_stopping = EarlyStopping(monitor='val_accuracy', patience=3, verbose=1, mode='max', restore_best_weights=True)

      hist = model.fit(train_dataset, epochs=epochs, callbacks = [checkpoint])

    # Testing
    model = load_model(checkpoint_path+f"/model_fold_{fold}.hdf5")
    with device:
